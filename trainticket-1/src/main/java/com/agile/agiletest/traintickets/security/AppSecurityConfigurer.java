package com.agile.agiletest.traintickets.security;
...
public class AppSecurityConfigurer extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
    	...
    	.antMatchers("/images/**","/css/**","/iconfont/**","/js/**","/login","/regist")
		//用户角色可访问的请求
		.permitAll().antMatchers("/index","/perInfo").hasAuthority("ROLE_USER")
		//改
	  	...
	  	.formLogin()
				//登陆跳转页面
		.loginPage("/login").successHandler(appAuthenticationSuccessHandler)
	  	.usernameParameter("username").passwordParameter("password")
				//登陆失败跳转
		.failureForwardUrl("/regist")
	  	.and()
	  	.logout().permitAll()
	  	.and()
		//加了可以跳转表单
